	...
	for await (const a of arr_abastecimentos){
		for await (const viatura of viaturas){
			...
			if(viatura.id_viatura == a._id_viatura){
				...
				if(viatura.ev){
					await viatura.abastecimentos.novo_ev([a._id, a._id_turno, a._kms, a._h_ini, a._carga_ini, a._h_fim, a._carga_fim, a._euros ]);
				}else{
					// const [dia, hora] = await a._data.split(' ');
					// const d = await (dia==undefined ? 'n/d' : dia);
					// const h = await (hora==undefined ? 'n/d' : hora);
					await viatura.abastecimentos.novo_mt([a._id, a._id_turno, a._data, a._kms, a._litros, a._euros]);
				}
